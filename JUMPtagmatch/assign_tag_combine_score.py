...
#     start = time.time()
    ...
    #             comet_evalue = get_comet_scores("xcorr", val["search_score"])
                ...
                if (jump_key in scan_tag_dictionary.keys()) & (modifications != []):
                    #print (spectrum)
                    #print (comet_evalue)
                    #modifications = val["modifications"] #list of dictionary
                    #print ("     modifications information {}".format(modifications))

                    all_mods = make_mod_col(modifications)
                    peptide = val["peptide"]
                    modifications_combine = ",".join(all_mods)
                    #print ("     comet_spectrum {}".format(comet_spectrum))
                    #print ("     jump_spectrum {}".format(jump_key))
                    #print ("     modifications output {}".format(modifications_combine))
