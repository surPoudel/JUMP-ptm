...
print("Keeping Rank1 psms with ptms only")
print("    Initial spectrum # {}".format(dfMz.shape[0]))
dfMz_valid,spectrum_list = keep_dynamic_mods(dfMz, var_mass)
# print (spectrum_list)

#selectRanks(dfMz_valid)
#dfMz_valid = dfMz_valid.loc[dfMz_valid.spectrum=="w010.02577.02577.2"]

print("Storing tag information and generating outfile links from comet to jump")
...
print("    THe mean of tag score of all tags is {}".format(tag_mean))
print("    THe standard deviation of tag score of all tags is {}".format(tag_std))



print("    ptm containing rank1 psms retained")
print("    Total retained spectrum # {}".format(dfMz_valid.shape[0]))
...
for df_x in df_split:
    ...
    jump_spectrum_list = get_tag_matched(df_x, scan_tag,jump_comet_link,all_matched_tags,int(min_tag_length))
    print("    total spectrums matched {} ".format(len(jump_spectrum_list)))
...
print("    Reporting tags per spectrum in the tag output file")
...
makePepXML(dfMz_valid, pep_xml,"{}/{}.pep.xml.tag_matched".format(output_directory,fraction_name), spectrum_tag_total_dict, logFile)




#weightedEvalue = combination of zscore(Escore = -log10(expect) and tagScore)
df = qc_target_decoy(dfMz_valid, "search_hit", "all", "num_matched_tags")
print("  The num_matched_tags qc numbers are shown below")
qc_figures(df, "num_matched_tags", "Total_Tag_matched")


df = qc_target_decoy(dfMz_valid, "search_hit", "all", "expect")
print("  The comet expect value qc numbers are shown below")
qc_figures(df, "-log10(eval)", "expect_minusLog10")


df = qc_target_decoy(dfMz_valid, "search_hit", "all", "xcorr")
print("  The comet xcorr value qc numbers are shown below")
qc_figures(df, "xcorr", "xcorr")

print("    Total Targets and Decoys ",df.Type.value_counts())
