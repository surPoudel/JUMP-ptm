...
def Get_params(jump_params):
	...
	params['mass_correction'] = int(float(jumpParamsDict['mass_correction']))	# 0 = no correction, 1 = MS1-based, 2 = MS1/2-based, 3 = manual correction
	params['prec_window'] = float(jumpParamsDict['prec_window'])	# 0 = disable; 1-10 (Da) = mz windows for removing precursor ions
	params['MS2_deisotope'] = int(float(jumpParamsDict['MS2_deisotope']))	# 0 = disable; 1 = enable
	params['ppm'] = float(jumpParamsDict['ppm'])	# mass tolerance for MS2 decharging and deisotoping
	params['ms2_consolidation'] = int(float(jumpParamsDict['ms2_consolidation']))	# maximal number of peaks retained within each 100-Da window
...
def CheckAdjMS1scans(params,ms1_mode,cur_premz,cur_ms1pos,sel_pnum,sel_pos,sel_mz,sel_inten,logfile):
	...
	[isonum,isomz,isointen,chg] = GetAllClustersOnSingleMS1(mz_m,inten_m,charge_list,unitdiff,ptola*2)
	inum=size(isomz,0)
	if 0==inum:
		if nrunning_mode==4:
			logfile.write('\n----------------CheckAdjMS1scans----------------\n')
	...
	#topn = 1
	[isonum,isomz,isointen,chg]=FilterByPPI(isonum,isomz,isointen,chg,1,cur_premz,ptola*2)
	c_len=int(isonum)
...
def GetAllClustersOnSingleMS1(mz,inten,charge_list,unitdiff,ptol):
		...
		for charge in charge_list:
			...
			while 1:
				...
				if len(pos)!=0:
					...
					# if ( (charge>2 and tp_isonum>=3) or (charge==2 and tp_isonum>=2) ) and tp_isointen[tp_isonum-1]/inten[pos[ix]]<0.6:# cut into two clusters
						tp_isonum=tp_isonum-1
						break
...
def FilterByPPI(isonum,isomz,isointen,chg,ncase,cur_premz,ptol):
	...
	if ncase==1:
		...
		if len(x1)>0:
			...
			if x[0]!=x1[xa[0]] and flag_Da[x[0]]==1 and isoclusterint[x[0]]/isoclusterint[x1[xa[0]]]>20:# much higher than the selected precursor
				isonum=isonum[x]
				isomz=isomz[x,:]
				isointen=isointen[x,:]
				chg=chg[x]
		...
		for jno in range(ino+1,mz_len):
			...
			if abs(i_isomz[0]-j_isomz[0])<=c_ptol:
				if i_chg==j_chg and i_isointen[0]<=j_isointen[0]:
					...
					flag[jno]=0
				elif mod(j_chg,i_chg)==0 or mod(i_chg,j_chg)==0:
					if Dscore[ino]<Dscore[jno]:
						flag[ino]=0
					else:
						flag[jno]=0
			elif len(ix)>0 and j_chg>i_chg and mod(j_chg,i_chg)==0:
...
def GetTopPPI(isonum,isomz,isointen,chg,Dscore,delta_Dscore,suminten,isogrp,max_num_ppi,percentage_ppi):
	...
	for ino in range(0,len(top_grps)):
		...
		for jno in range(0,len(ix)):
			...
			monoppi.append(suminten[ix[jno]]/sum(isoclusterint))
...
def deisotope(mz0_ext,inten0_ext,mz0,inten0,pno,charge,unitdiff,ptol,procflag):
	...
	while search_loop>=1 and flag==1:
		...
		pos=nonzero((mz0>=left) & (mz0<=right) & (procflag==0))[0]
		if len(pos)>0:
			flag=1
...
def GetClusters(MS1_index,MS1_peaks,index,params,all_tIPV,ms1_mode,nrunning_mode,cur_premz,cur_ms1pos):
	...
	if nrunning_mode==1 or nrunning_mode==2:
		...
		[sel_pnum,sel_pos,sel_mz,sel_inten] = CheckAdjMS1scans(params,ms1_mode,cur_premz,cur_ms1pos,sel_pnum,sel_pos,sel_mz,sel_inten,'')
		[mz,inten] = GetRefMS1(sel_pnum,sel_mz,sel_inten,ptole)
		[isonum,isomz,isointen,chg] = GetAllClustersOnSingleMS1(mz,inten,charge_list,unitdiff,ptola*2)
		...
		for cno in range(0,len(cmz)):
			for dno in range(0,len(charge_list)):
				gno=grp[cno]+dno+1
				for eno in range(0,len(ion_shift)):
					monomz.append(cmz[cno]+ion_shift[eno]*unitdiff/charge_list[dno])
					monochg.append(charge_list[dno])
...
def GetClusters_psm(MS1_index,MS1_peaks,index,params,all_tIPV,ms1_mode,nrunning_mode,cur_premz,cur_ms1pos,cur_ms2scan,logfile):
	...
	if nrunning_mode==4:
		if len(psm)>0:
			...
			grp=[]
			[sel_pnum,sel_pos,sel_mz,sel_inten] = SelectPeaks(MS1_index,MS1_peaks,index,params,ms1_mode,cur_premz,cur_ms1pos)
			[sel_pnum,sel_pos,sel_mz,sel_inten] = CheckAdjMS1scans(params,ms1_mode,cur_premz,cur_ms1pos,sel_pnum,sel_pos,sel_mz,sel_inten,logfile)
			[mz,inten] = GetRefMS1(sel_pnum,sel_mz,sel_inten,ptole)
			[isonum,isomz,isointen,chg] = GetAllClustersOnSingleMS1(mz,inten,charge_list,unitdiff,ptola*2)
			[isonum,isomz,isointen,chg]=FilterByPPI(isonum,isomz,isointen,chg,topN,cur_premz,ptola*2)
			if size(isomz,0)>0:
