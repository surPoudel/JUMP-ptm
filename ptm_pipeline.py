...
#tag program
jump_tag_program = "python {}/tagmatch_program_rank1/main_pho.py".format(source_path)
...
if pitfile == "0": #need to apply jump -d to generate protein inference table (pit) file
    ...
    os.chdir("database")
    jump_d_params = "/home/spoudel1/PanPTM_Paper_2021/pt_pipeline_tag_added/parameterFiles/jump_d.params"
...
if idtxt_file != "0":
    ...
    #check if stage 0 results are available. If available than skip stage_0 searching and filtering else perform stage 0
  
    ptm_params = "Stage_0/stage_0_comet.params"
    write_log (logFile," \n\n****** Generating Parameter files *********\n")
    write_log (logFile,"  Working for stage 0")
    write_log (logFile,"  This stage will search unmodified peptide for the Stage 0 whole proteome using comet search engine")
    write_log (logFile,"  After the search a default FDR = 1.5 at the protein level will be used for JUMP -f filtering")
    ...
    write_log (logFile,"  Stage_0 -- unmodified whole search, comet parameter file is generated and stored in {} folder".format(os.getcwd()))
...
if idtxt_file == "0":
    write_log (logFile,"\n\nThis part of log file has the preprocessing information. Preprocessing is done if jump -f results are not provided for the whole proteome. Comet will be used as the search engine")

    #use in house preprocessing program to perform stage 0 search or whole proteome searches
    for mz_file in mzXMLs:
        os.chdir(preprocess)

        cpFile(params_file, "preprocess.params")
        softlink_mzxml([mz_file], ".")
        cmd = "{} preprocess.params ".format(deisotope_program)+os.path.basename(mz_file)
        os.system(cmd)
        write_log (logFile,"  In-house preprocessing program initiated for the default TMT dataset")
    
    
    #wait until all the files are completely preprocessed
    #the preprocessing program submits 4 jobs right now (so we will have 4* 4 log.out files)
    #so we have to wait but since python do not wait, we need to somehow check the program 

    wait_Function(4, preprocess, mzXMLs)
    write_log (logFile,"  Preprocessing for {} mzxml files is complete. Respective dtas and ms2 are created in the {} folder".format(len(mzXMLs), os.getcwd()))
    #search pipeline for stage 0 ____ automatic searching happens 
    
    write_log (logFile,"\n\nThis part of logFile consists of Stage_0 (whole unmodified search information")
    
    stage_wise_search_server(stage0_folder, mzXMLs, comet,logFile, preprocess = preprocess)
# #     stage_wise_search(stage0_folder, mzXMLs, comet, preprocess)
    wait_Function(1, stage0_folder, mzXMLs)
    
    write_log (logFile,"  Formating pepxml suffix to match standard jump output for Stage_0 results")

    rename_pep_xml(mzXMLs, stage0_folder)
...
if idtxt_file != "0":
    ...
    #get the list of all_tags_file
    #all_tags_file = glob.glob(stage0_folder+"/*/*.tags")
else:
    ...
    write_log (logFile,"  Following parameters are internally defined for filtering Stage_0")
    write_log (logFile,"  min_XCorr=1 (Comet -search specific)\n  search_engine=comet\n  pitfile={}\n  database={}".format(pitfile, database_name))


    #prepare_jump_f_input(folder, fdr, pitfile, stage, search_engine, min_XCorr)

    
    out_params = prepare_jump_f_paramsFile(stage0_folder, allParamsDict_f, os.path.basename(stage0_folder), fdr)



    # out_params,list_pubs, fdr_add, pitfile_str,database_str, search_engine_str, min_XCorr_str = prepare_jump_f_input(stage0_folder, fdr, pitfile, database_name,os.path.basename(stage0_folder),"comet", "1")#search engine and min
    # from_sample_to_batch_params(jump_f_params, out_params, list_pubs, fdr_add, pitfile_str, database_str, search_engine_str, min_XCorr_str)

    # cmd = "jump -f "+out_params
    cmd = "{} {}".format(jump_f_program,out_params)
    os.system(cmd)
    jump_f_id_txt = "{}/sum_Stage_0_FDR_{}/ID.txt".format(stage0_folder, fdr)


    ####For QC spectrum analysis ####THIS PART IS STILL UNDER DEVELOPMENT####
    if enable_spectrum_QC == "1":
        fdr_add = "FDR = 0"
        fdr = "0"
        ...
        #this is required to make the parameter file
        #confident_IDtxt, accepted_IDtxt
        qc_id_txt_list = [jump_f_id_txt,jump_f_id_txt_0]

        #generate QC parameters
        generate_QC_param_file(stage0_folder, "jump_qc.params", qc_id_txt_list, PSM_recoveray_rate)
...
if idtxt_file != "0":
    #with qc path or without qc path undergoing qc we have qc path = dtasFolder
    jump_f_qc_0_idtxt = dirname(dtasFolder)+"/sum_Stage_0_FDR_0/ID.txt" #this ID.txt is from qc fdr = 0 
    stage0_idtxt = jump_f_qc_0_idtxt
else:
    stage0_idtxt =  glob.glob(curr_path+"/{}/Stage_0/sum_*/ID.txt".format(result_folder))[0]

write_log (logFile,"  Concatenating jump -f ID.txt")
head1,df_idtxt_all = combine_jump_f_stages_modPeptides(all_stage_idtxts, stage0_idtxt)
...
df_uni_pep = merge_publication_jump_f_tables(all_stage_id_uni_pep, merge_directory, stage0_idtxt, keep_spectrum)
write_log (logFile,"  Concatenating jump -f id_all_pep.txt")
df_all_pep = merge_publication_jump_f_tables(all_stage_id_all_pep, merge_directory, stage0_idtxt, keep_spectrum)
