...
#		printf "\rCreating proteinhash with Peptide_Hash: %d%% done", ($peptidenum/$pepnum)*100;
		...
		{
			if ($protein eq '') {
				warn "blank protein accession for $peptide\n";
				next;
			}
			...
			if (!defined($$proteinhash{$protein}))
			{
				...
				if ( length($$proteinhash{$protein}{'sequence'})==0 ){
					#die "create_proteinhash(): Zero length protein:$protein!!!\n";
				}
			...
			#Branched-chain-amino-acid aminotransferase OS=Drosophila melanogaster OX=7227 GN=Dmel\CG1673 PE=1 SV=2  
			if ($$proteinhash{$protein}{'annotation'} =~ m/\sGN=(.*)\s/) # revised on 8/24/20: to adapt Fly database (example above)
			{
...
sub find_XCorrdCn_cutoffs
{
	...
	$$paramhash{group_by_NmatchedTags}=1;
	#$$paramhash{group_by_pepLength}=1;
	#$$paramhash{group_by_tryp}=1;
	#$$paramhash{group_by_mod}=1;
	#$$paramhash{group_by_mis}=1;
	#$$paramhash{group_by_charge}=1;
	#$$paramhash{group_by_dCn}=1;
	...
	while (my ($intpep, $intpephash) = each %$peptidehash)
	{
		...
		while (my ($outfile, $outhash) = each %{$$intpephash{'outfiles'}})
		{
			...
			# define factors: from most importatn to least:
			if ( defined($$paramhash{group_by_NmatchedTags}) and $$paramhash{group_by_NmatchedTags} ) {
				if ($$outhash{'num_matched_tags'} ne '') {
					my $num_matched_tags=$$outhash{'num_matched_tags'};
					$num_matched_tags = 9 if ($num_matched_tags>9);
					$ps.= 9 - $num_matched_tags; # single digit; the smaller, the better
				}
			}
#=head
			my $nomod=$intpep; # top factor: peptide length
...
=head
  ...
  if($database=~/hdr/)	{$database=~s/\.hdr$//;	}
=cut
  my $database = $$paraHash{'DB'};
  #my $mass = $$outInforHash{$outkey}{'calc_neutral_pep_mass'}[1]+$Hydrogen_mass+$$outInforHash{$outkey}{'massdiff'}[1];
  my $mass = $$outInforHash{$outkey}{'precursor_neutral_mass'}+$Hydrogen_mass; # observed/detected mono mass
  ...
  # added on 3/7/2022: tag QC for comet pep.xml
  my $num_matched_tags='';
  if (defined($$outInforHash{$outkey}{'num_matched_tags'}[$k])) {
	$num_matched_tags = $$outInforHash{$outkey}{'num_matched_tags'}[$k];
  }
...
#	if ($expmass != $calcmw) { die "$outfile: $expmass, $calcmw, $peptide\n"; }	
  #$expmass = $calcmw;#$expmass predefined; why bothers recalculation? why diff?
  $expmass = $$outInforHash{$outkey}{'calc_neutral_pep_mass'}[1]+$Hydrogen_mass; # directly extracted from search resutls (pepXML)
...
#
 	...
 	$$runhash{$run}{$outfile}{'scan'} = $scan;
       ...
       $$runhash{$run}{$outfile}{'peptide'} = $peptide;
        $$runhash{$run}{$outfile}{'tryptic'} = $tryptic;
        $$runhash{$run}{$outfile}{'num_matched_tags'} = $num_matched_tags;
        $$runhash{$run}{$outfile}{'mod'} = $mods;
        $$runhash{$run}{$outfile}{'mis'} = $mis;
        $$runhash{$run}{$outfile}{'red'} = $red;
        $$runhash{$run}{$outfile}{'charge'} = $charge;
  ...
  $$peptidehash{$intpep}{'outfiles'}{$outfile}{'peptide'} = $peptide;
  $$peptidehash{$intpep}{'outfiles'}{$outfile}{'tryptic'} = $tryptic;
  $$peptidehash{$intpep}{'outfiles'}{$outfile}{'num_matched_tags'} = $num_matched_tags;
  $$peptidehash{$intpep}{'outfiles'}{$outfile}{'mod'} = $mods;
  $$peptidehash{$intpep}{'outfiles'}{$outfile}{'mis'} = $mis;
  $$peptidehash{$intpep}{'outfiles'}{$outfile}{'red'} = $red;
  $$peptidehash{$intpep}{'outfiles'}{$outfile}{'charge'} = $charge;
