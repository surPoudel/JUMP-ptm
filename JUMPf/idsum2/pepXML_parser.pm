...
sub pepXML2Hashes
{
	...
	while (<IN>)
	{
		...
		# N-term dyn mod
		# <terminal_modification terminus="N" massdiff="229.162931" mass="230.170756" variable="Y" protein_terminus="N" symbol="@"/>
		if (/terminal_modification terminus\=\"[Nn]\" massdiff=\"([\d\.]+)\" mass=\"([\d\.]+)\" variable=\"Y\" protein_terminus=\"[Nn]\" symbol=\"(.*)\"\/\>/)
		{			
			$$paraHash{'NtermMod_dyn'}{'massdiff'}=$1;
			$$paraHash{'NtermMod_dyn'}{'mass'}=$2;
			$$paraHash{'NtermMod_dyn'}{'symbol'}=$3;
			#print "N-term dyn mod: check: $$paraHash{'NtermMod_dyn'}{'symbol'}\n";
			next;
		}
	...
	my %scanPrecursorHash; # added on 1/7/2022: to solve Comet pepXML issue (same scan with multiple precursor but the same outfile name)
				# hash structure: %scanPrecursorHash{$scan}{$precursor}=$precursorIndex; # here $scan = "$fraction.$scan"
	while (<IN>)
	{
		my %tmpOutHash; # temporary outhash to hold values (before getting renamed outfile

		# <spectrum_query spectrum="w002.20069.20069.2" start_scan="20069" end_scan="20069" precursor_neutral_mass="2227.055196" assumed_charge="2" index="68">
		#if (/\<spectrum_query /)
		if (/\<spectrum_query /)
		{
			my @tmp = / (\w+)\=\"(.*?)\"/g; # split by pattern: field = "value"
			my $outfileRaw=$tmp[1];
			# get field-value pairs
			for (my $i=0; $i<scalar(@tmp); $i=$i+2)
			{
				$tmpOutHash{$tmp[$i]}=$tmp[$i+1]; 
			}

			# update %scanPrecursorHash
			# w002.20069.20069.2
			unless ($outfileRaw =~ m/^(.*?)\.(\d+)\.(\d+)\.(\d)$/) {die "Unexpected outfile name: $outfileRaw\n@tmp\n";}
			$outfileRaw =~ m/^(.*?)\.(\d+)\.(\d+)\.(\d)$/;
			if (0) {
			#if ($2 == $3) { # this condition statement may be bug-prone: this will cause issues for the small scan # e.g., 1, 2, 3, etc.
				my $scan = "$1.$2"; # $scan = "$fraction.$scan"
				my $precursor_neutral_mass = $tmpOutHash{precursor_neutral_mass};
				my $charge = $4;
				if (!defined($scanPrecursorHash{$scan})) {
					$scanPrecursorHash{$scan}{$precursor_neutral_mass} = 1;
				} else {
					my $lastIndex = scalar(keys %{$scanPrecursorHash{$scan}});
					$scanPrecursorHash{$scan}{$precursor_neutral_mass} = $lastIndex+1;
				}

				# define $outfile name
				my $precursorIndex = $scanPrecursorHash{$scan}{$precursor_neutral_mass};
				$outfile="$scan.$precursorIndex.$charge";
			} else {
				$outfile=$outfileRaw;
			}

			# attach field-value to %outInforHash
			for (my $i=0; $i<scalar(@tmp); $i=$i+2)
			{
...
=cut		
		...
		#modifications
		# N-term dyn mod?
		# w/ dyn mod:  <modification_info modified_peptide="n[230]K[170]LEAAEER" mod_nterm_mass="230.170756"> 
		# w/o dyn mod: <modification_info modified_peptide="KSSFPETEEK[170]VRR">
		if (/\<modification_info modified_peptide=/) {
			if (/mod_nterm_mass=\"(.*?)\"/) {
				$$outInforHash{$outfile}{NtermMod_dyn} = 1;
			} else {
				$$outInforHash{$outfile}{NtermMod_dyn} = 0;
			}
			next;
		}
		# ordinary AA dyn mod
	...
	} # end of reading pepXML
	close IN;
	#print $$outInforHash{'adtmt1mg3.727.727.2'}{'deltacn'}[2];

	# LAST TASK: to clue with the old idsum program, we need to reconstruct full peptide with dynamic modifications
	foreach my $outfile (keys %{$outInforHash})
	{
		for (my $i=1; $i<=$$outInforHash{$outfile}{'hitShown'}; $i++)
		{
			# w/ ordinary AA dyn mod
			...
			# Added in 2020 Aug.: consider N-term dyn mod
			# if N-term mod: add 'J*'   # * is just example symbol
			# else: add 'J'
			if (defined($$paraHash{'NtermMod_dyn'}{'massdiff'}) and $$paraHash{'NtermMod_dyn'}{'massdiff'} ne '') {
				# step 1: define N-term symbol
				my $Nterm_symbol='J';
				# step 2: add symbol (w/ two possibilites)
				if ($$outInforHash{$outfile}{NtermMod_dyn}) {
					$$outInforHash{$outfile}{'fullPeptide'}[$i]=join('',$Nterm_symbol,$$paraHash{'NtermMod_dyn'}{symbol},$$outInforHash{$outfile}{'fullPeptide'}[$i]);
				} else {
					$$outInforHash{$outfile}{'fullPeptide'}[$i]=join('',$Nterm_symbol,$$outInforHash{$outfile}{'fullPeptide'}[$i]);
				}
			}
		}
	}
