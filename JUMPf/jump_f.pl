...
#						$bad++; $brandom++ if ($$hash{'protein'}=~/^Random__/ || $$hash{'protein'} =~ /^Decoy__/);
		...
		# clean up %peptidehash (remove zombies)
...
sub pep_pos{
	...
	while (my ($pro, $prohash) = each %$hash){
		...
		my $proseq = $$prohash{'sequence'};
		if ($proseq eq '') {die "pep_pos(): Zero length protein:$pro\n\n";}
...
sub coverage{
	...
	while (my ($pro, $prohash) = each %$hash){
  	...
  	##printf "$num vs %d\n", length($Seq);exit;
  	if ($Seq eq '') {die "coverage(): Zero length protein:$pro\n$Seq\n";}
        ...
        #while (my ($run,$runhash) = each %scanhash){# %runhash{$scan}{$outfile}
                #for my $hash (values %$runhash){# %hash{$outfile}
					...
					unless ($$hash{$outfile}{'charge'} == 3 or $$hash{$outfile}{'charge'} == 2) {
						warn "Error: For  +2/+3 charge assignments of uncharged precursor ions (rm_multiple_charge_for_single_precursor), unexpected charge state was found: $outfile, charge == $$hash{$outfile}{'charge'}\n";
						next;
					}
