...
if len(mzXMLs) == 0:
    ...
    for mz_file in mzXMLs_pre:
        cpFile(params_file, "preprocess.params")
        softlink_mzxml([mz_file], ".")
        cmd = "{} preprocess.params {}".format(deisotope_program, mz_file)
        os.system(cmd)

    mzXMLs = glob.glob("{}/*/*.ms2".format(curr_path))
    # print ("        The new input is {}".format(mzXMLs))
    ...
    stage_wise_search(stage0_folder, mzXMLs, comet, logFile, cluster, scanChargeDf)
...
if idtxt_file != "0":
    ...
    os.system(cmd)
    idtxt_file = "{}/sum_Stage_0_FDR_{}/ID.txt".format(stage0_folder, fdr)

accepted_protein_list, scanChargeDf = idtxt_exclude_acceptedPSMs_acceptedProtein(idtxt_file)
